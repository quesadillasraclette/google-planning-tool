...
export class ContentCalendar extends Component {
  ...
  getListData = value => {
    let listData;
    const { contents } = this.state;
    listData = contents.filter(
      element =>
        value.date() === element.publishDate.date() &&
        value.month() === element.publishDate.month() &&
        value.year() === element.publishDate.year()
    );
      ...
      <ul className="events">
        {listData.map((item, key) => (
          <Link key={key} to={`content/${item.content.id}`}>
            <div>
              <Tooltip title={`${item.content.budget} ${item.content.channel}`}>
                <span>
